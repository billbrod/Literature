...
# files will be saved directly in this directory (as literature.bib
# and literature.org, respectively), each reference will be given a
...
def setup_folders_nofile(bib):
    ...
    if not os.path.isfile(paper_dir + '/' + bib['ID'] + '/' + bib_path):
        ...
        print('Found a bib file at %s, not saving new one' %
              (paper_dir + '/' + bib['ID'] + '/' + bib_path))
...
def setup_folders_withfile(file_path, bib):
    ...
    new_path = '{bib_id}{extension}'.format(
        bib_id=bib['ID'], extension=(os.path.splitext(file_path)[1]).lower())
...
def master_org_add(bib_path):
    ...
    if bib_path[0] != '/':
        bib_path = paper_dir + '/' + \
            os.path.splitext(bib_path)[0] + '/' + bib_path
    ...
    new_org = re.subn('\[\[file:(.*)\.(.*)\]\]',
                      r'[[file:\1/\1.\2]]', new_org)[0]
...
if __name__ == '__main__':
    ...
    if (os.path.splitext(sys.argv[1])[1]).lower() == '.bib':
        ...
        raise Exception('Don\'t know how to deal with extension %s' %
                        os.path.splitext(sys.argv[1])[1])
