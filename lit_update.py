...
def update(fill_column=70):
    ...
    import bibtexparser
    import re
    from lit_add import paper_dir, org_format, git_update_commit
    ...
    master_org = [(i, j, key_get(j))
                  for i, j in zip(master_org[::2], master_org[1::2])]
    ...
    deleted_files = []
    modified_time = {'bib': os.path.getmtime(paper_dir + '/literature.bib'),
                     'org': os.path.getmtime(paper_dir + '/literature.org')}
    for bib_idx, bib in enumerate(master_bib.entries):
        ...
        if not os.path.isdir(dir_path):
            ...
            print("Directory for bib id %s not found, "
                  "removing from master bib and org files..." % bib_id)
            master_org_idx = [bibid for (
                i, j, bibid) in master_org].index(bib_id)
            master_org.pop(master_org_idx)
            deleted_files.extend(
                [paper_dir + '/{bib}/{bib}.{ext}'.format(bib=bib_id,
                                                         ext=extension)
                 for extension in ['org', 'pdf', 'bib']])
            ...
            continue
        if (os.path.isfile('%s/%s.pdf' % (dir_path, bib_id)) and
            os.path.getmtime('%s/%s.pdf' % (dir_path, bib_id)) >
                modified_time['org']):
            print('Pdf %s.pdf updated, extracting annotations' % bib_id)
            annots = get_annotations(
                '%s/%s.pdf' % (dir_path, bib_id), 'org',
            ...
            org_note = re.split('^([*]+ )(.*)', org_note,
                                flags=re.MULTILINE)[1:]
            org_note = [[hdr_lvl, title, txt_body] for hdr_lvl, title, txt_body
                        in zip(org_note[::3], org_note[1::3], org_note[2::3])]
            annotation_idx = [title.strip() for hdr_lvl, title,
                              txt_body in org_note].index('Annotations')
        ...
        if (os.path.getmtime('%s/%s.org' % (dir_path, bib_id)) >
                modified_time['org']):
            print("Org file %s.org updated, copying new "
                  "changes to master org" % bib_id)
            ...
            org_note = re.subn('\[\[file:(.*)\.(.*)\]\]',
                               r'[[file:%s/\1.\2]]' % bib_id, org_note)[0]
            org_note = re.split('^[*] ([A-Z]+)', org_note,
                                flags=re.MULTILINE)[1:]
            ...
            if "%s/%s.org" % (dir_path, bib_id) not in updated_files:
                updated_files.append("%s/%s.org" % (dir_path, bib_id))
        if (os.path.getmtime('%s/%s.bib' % (dir_path, bib_id)) >
                modified_time['bib']):
            print("Bib file %s.bib updated, copying new "
            ...
            bib_tmp['notefile'] = re.sub(
                '(.*)\.org', r'%s/\1.org' % bib_id, bib_tmp['notefile'])
...
def get_annotations(path, note_format='plain', org_indent='   ',
    ...
    assert(note_format in ['plain', 'wrap', 'org'],
           "Don't know how to format annotations with "
        ...
        for page_no, page in enumerate(pages):
            ...
            for j in items:
                ...
                if note_format == 'plain':
                    ...
                    j = col_wrap('%s (page %s)' %
                                 (j, page_no + 1), fill_column)
                    notes = notes + '%s\n\n' % j
                elif note_format == 'org':
                    j = col_wrap('%s (page %s)' %
                                 (j, page_no + 1), fill_column, org_indent)
                    notes = notes + '%s%s\n\n' % (org_indent, j)
        return notes
...
def force_renew(fill_column=70, org_flag=False, bib_flag=False):
    ...
    if org_flag:
        if os.path.isfile(paper_dir + '/literature.org'):
            continue_choice = raw_input(
                "Master org file exists, continue? "
                "THIS WILL REMOVE THE FILE AND RECREATE IT [y/n] ").lower()
            if continue_choice == 'y':
                os.chmod(paper_dir + '/literature.org',
                         stat.S_IWUSR | stat.S_IREAD)
        ...
        if not os.path.isfile(paper_dir + '/literature.org'):
            ...
            for org_file in org_glob:
                ...
                assert(len(bib_id) == 1,
                       "Found zero or more than 1 bib_ids (%s) for "
                       "file %s" % (bib_id, org_file))
                tmp = re.subn('\[\[file:(.*)\.(.*)\]\]',
                              r'[[file:%s/\1.\2]]' % bib_id[0], tmp)[0]
                master_org_text += "\n".join(tmp.split("\n")[1:]) + "\n\n"
            with open(paper_dir + '/literature.org', 'w') as f:
    ...
    if bib_flag:
        ...
        if not os.path.isfile(paper_dir + '/literature.bib'):
            ...
            print("literature.bib exists and "
                  "you don't want to over-write it...")
